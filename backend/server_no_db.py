...
# TMDB Service Class (same as before)
class TMDBService:
    def __init__(self):
...
# In-memory search history (for demo without database)
search_history = []
...
async def search_movies(request: MovieSearchRequest):
    ...
    try:
        ...
        for item in tmdb_results.get("results", [])[:15]:
            try:
                if item.get("media_type") == "movie":
                    try:
                        ...
                        logger.warning(f"Failed to get movie details: {e}")
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to get TV details: {e}")
...
async def get_search_history():
    ...
    try:
        # Return from memory (replace with database later)
        recent_history = sorted(search_history, key=lambda x: x["timestamp"], reverse=True)[:10]
        return {
